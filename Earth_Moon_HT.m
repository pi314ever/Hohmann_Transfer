...
tspantr = [0 2*pi*sqrt(a^3/(G*Me))]*30+sol1.x(end);
...
x02 = soltr.y(:,end);
uv2 = 0*x02(4:6)/norm(x02(4:6));
x02(4:6) = sqrt(G*Mm/R2mag(x02))*uv2;
tspan2 = [0 pi/6*rf/sqrt(G*Me/rf)/100]+soltr.x(end);
opt2 = odeset('Events',@(t,x)eventhit(t,x,xe,xm,Re,Rm)); % Stop if hit Moon or Earth
...
plot3(x(1),y(1),z(1),'x')
plot3(xtr(1),ytr(1),ztr(1),'x')
plot3(x2(1),y2(1),z2(1),'x')
